...
public class Simulatore {
	...
	public void init(Country partenza, Graph<Country, DefaultEdge> grafo) {
		...
		for(Country c : this.grafo.vertexSet()) {
			stanziali.put(c, 0);	//inizialmente il numero degli stanziali in ogni stato è 0
		}
	...
	public void run() {
		...
		while((e = queue.poll()) != null) {
			...
			int migranti = (nPersone/2)/confinanti.size();  //divisione troncata per difetto poichè migranti è un int
			...
			this.stanziali.put(stato, this.stanziali.get(stato) + stanziali);	
									  //Somma al numero degli stanziali già presenti i nuovi stanziali
		}
	}
